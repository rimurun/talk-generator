  ...
  const SidebarLink = ({ item }: { item: NavItem }) => {
    ...
    return (
      ...
      >
        ...
        <Icon
          size={18}
          className={`shrink-0 transition-colors ${active ? 'text-blue-400' : 'text-gray-500 group-hover:text-white'}`}
        />
      ...
      >
          ...
          >
            ...
            {!isCompact && (
              <div className="min-w-0">
                ...
                <p className="text-xs text-gray-500 truncate">配信用台本ツール</p>
              </div>
            )}
          </Link>
        </div>
        ...
        >
          {isCompact
            ...
            : <><PanelLeftClose size={14} /><span>折りたたむ</span></>
          }
        </button>
        ...
        <div className={`px-2 space-y-0.5 ${isCompact ? 'px-2' : 'px-3'}`}>
          {!isCompact && (
            <p className="px-2 pb-1 text-xs font-semibold text-gray-600 uppercase tracking-wider">
              メイン
            </p>
          )}
          {primaryNav.map((item) => (
        ...
        {/* セクション区切り */}
        <div className="mx-3 my-3 border-t border-gray-800/60" />
        ...
        <div className={`${isCompact ? 'px-2' : 'px-3'}`}>
          ...
          {!isCompact && (
            <button
              ...
              className="w-full flex items-center justify-between px-2 pb-1 text-xs font-semibold text-gray-600 uppercase tracking-wider hover:text-gray-400 transition-colors"
            >
              <span>ライブラリ</span>
              {isSecondaryOpen
                ? <ChevronDown size={12} />
        ...
        <div className="mt-auto">
          ...
          {/* リーガルリンク（展開時のみ表示） */}
          {!isCompact && (
            <div className="px-5 mb-2 flex gap-3 text-xs text-gray-600">
          ...
          {isConfigured && (
            <div className={`${isCompact ? 'px-2 pb-4' : 'px-3 pb-4'} border-t border-gray-800/60 pt-3`}>
              {user ? (
                // ログイン済み
                <div className={`${isCompact ? '' : 'bg-gray-900 rounded-lg p-2.5 space-y-1.5'}`}>
                  {!isCompact && (
                    <div className="flex items-center gap-2 text-xs text-gray-400">
                      <User size={13} className="shrink-0 text-gray-600" />
                  ...
                  <button
                    ...
                    className={`
                      flex items-center gap-1.5 text-xs text-gray-500 hover:text-red-400 transition-colors
                ...
                <Link
                  ...
                  className={`
                    flex items-center gap-2 text-xs text-gray-400 hover:text-white hover:bg-white/5
              ...
              {/* バージョン表記（展開時のみ） */}
              {!isCompact && (
                <div className="mt-2 px-1 text-xs text-gray-700">
              ...
              return (
                ...
                >
                  ...
                  <span className={`
                    ...
                    ${active ? 'tab-active-glow scale-110 text-blue-400' : 'text-gray-500'}
                  `}>
                    <Icon size={20} />
                    {/* アクティブドット */}
                    {active && (
