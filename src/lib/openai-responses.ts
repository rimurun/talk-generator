    ...
    properties: {
      topics: {
        ...
        items: {
          ...
          required: ['title', 'category', 'summary', 'publishedAt'],
        },
      },
    },
    required: ['topics'],
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      const allowedCategories = filters.categories.length > 0
        ? filters.categories
        : ['ニュース', 'エンタメ', 'SNS', 'TikTok', '海外おもしろ', ...(filters.includeIncidents ? ['事件事故'] : [])];
      ...
      // recency設定（カテゴリ別、複数の場合は最短を使用）
      const recency = filters.categories.length === 1
        ? (CATEGORY_RECENCY[filters.categories[0]] || 'day')
        : 'day';
...
${filters.includeIncidents ? '' : '事件事故は除外。'}テンション: ${filters.tension}
...
${previousTitles && previousTitles.length > 0 ? `除外（既出）: ${previousTitles.slice(0, 10).join(', ')}` : ''}`;
      ...
      // JSONパース
      let rawTopics: Array<{ title: string; category: string; summary: string; publishedAt: string }> = [];
      try {
...
// ===========================================================
// 台本生成（OpenAI Chat Completions API — 変更なし）
// ===========================================================
